...
export default function MechanicsStoresPage() {
    ...
    const handleBookSession = (item: any) => {
        setSelectedItem(item);
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <h1 className="text-4xl font-bold mb-8 text-center">Mechanics and Stores</h1>
            ...
            <Tabs defaultValue="all" className="mb-8">
                ...
                {['all', 'mechanics', 'stores'].map((tab) => (
                    <TabsContent key={tab} value={tab}>
                        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                            {mechanicsAndStores
                                .filter((item) => tab === 'all' || item.type.toLowerCase() === tab)
                                .map((item) => (
                                    <MechanicStoreCard
                                        key={item.id}
                                        item={item}
